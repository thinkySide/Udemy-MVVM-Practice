...
final class TIP_CalculatorTests: XCTestCase {
    ...
    func testSoundPlayedAndCaculatorResetOnLogoViewTap() {
        let input = buildInput(bill: 100, tip: .tenPercent, split: 2)
        let output = sut.transform(input: input)
        let expectation1 = XCTestExpectation(description: "reset calculator call")
        let expectation2 = audioPlayerService.expectation
        
        output.resetCaculatorPublisher.sink { _ in
